...
async function getEquiposFromAPI(page, searchTerm = '') {
    ...
    const filters = `include=prestamo&tipo[eq]=Accesorio&page=${page}&searchTerm=${searchTerm}&entriesPerPage=${entriesPerPage}`;
    try {
...
function newRow(accesorios) {
    ...
    Object.entries(valueMapping).forEach(([selector, property]) => {
        row.querySelector(selector).textContent = accesorios[property];
    });

    const estadoElement = row.querySelector(".estado");
    estadoElement.innerHTML = '<i class="fa-solid fa-circle fa-2xs my-auto mr-1"></i>' + accesorios.estado;
    ...
    estadoElement.classList.add(statusMapping[accesorios.estado]);
    row.querySelector('.open-changePhoto-modal').addEventListener("click", () => showChangePhotoModal(accesorios));
    row.querySelector('.open-viewObser-modal').addEventListener("click", () => showObservationsModal(accesorios.id, accesorios.observaciones));
    row.querySelector('.open-changeStatus-modal').addEventListener("click", () => showChangeStatusModal(accesorios.id, accesorios.estado));
    row.querySelector('.open-delete-modal').addEventListener("click", () => showDeleteModal(accesorios.id));
    row.querySelector('.open-prestamos-modal').addEventListener("click", () => showPrestamoModal(accesorios));
    row.querySelector('.open-edit-modal').addEventListener("click", () => showEditModal(accesorios));
...
function showPrestamoModal(accesorio) {
    ...
    noResults.style.display = 'none';
        ...
        row.querySelector('.id').innerText = accesorio.prestamo.id;
        row.querySelector('.estado').innerText = `${accesorio.prestamo.estado}`;
        row.querySelector('.fecha').innerText = accesorio.prestamo.fecha_prestamo.split('-').reverse().join('/');
        row.querySelector('.estado').innerText = accesorio.prestamo.estado
